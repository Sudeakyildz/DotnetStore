...
import { apiFetch, readJson, getStoredAuthProfile } from '../api/client';
...
const Dashboard = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (role === AuthRoles.Admin || role === AuthRoles.StaffCategories) {
        requests.push(apiFetch('/api/Categories'));
      }
      if (role === AuthRoles.Admin || role === AuthRoles.StaffPrices || role === AuthRoles.StaffFeatures) {
        requests.push(apiFetch('/api/Products'));
      }
      if (role === AuthRoles.Admin || role === AuthRoles.StaffFeatures) {
        requests.push(apiFetch('/api/Features'));
      }
      ...
      if (role === AuthRoles.Admin || role === AuthRoles.StaffCategories) {
        const cRes = results[ci++];
        if (cRes.ok) categories = await readJson<CategoryDto[]>(cRes);
      }
      if (role === AuthRoles.Admin || role === AuthRoles.StaffPrices || role === AuthRoles.StaffFeatures) {
        const pRes = results[ci++];
        if (pRes.ok) products = await readJson<ProductListItemDto[]>(pRes);
      }
      if (role === AuthRoles.Admin || role === AuthRoles.StaffFeatures) {
        const fRes = results[ci++];
        if (fRes.ok) features = await readJson<FeatureDto[]>(fRes);
      }
  ...
  const showCat = role === AuthRoles.Admin || role === AuthRoles.StaffCategories;
  const showProd = role === AuthRoles.Admin || role === AuthRoles.StaffPrices || role === AuthRoles.StaffFeatures;
  const showFeat = role === AuthRoles.Admin || role === AuthRoles.StaffFeatures;
      ...
      <header className="dashboard-hero mb-4 mb-md-5">
        ...
        <p className="dashboard-hero-meta text-muted small mb-0">
          ...
          {auth && (
            <>
              {' '}
              · <span className="text-secondary">{auth.userName}</span> ({auth.role})
            </>
          )}
        </p>
      </header>
          ...
          <div className="col-md-4">
            <Link to="/products" className="text-decoration-none dash-tile h-100 d-block">
              <div className="dash-tile-inner dash-tile-inner--accent h-100">
                ...
                <p className="dash-tile-desc">Liste, arama ve fiyat / özellik (yetkinize göre)</p>
                <span className="dash-tile-action">
                  Aç
                  <span className="dash-tile-chevron" aria-hidden>
                    →
